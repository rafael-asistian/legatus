import { Head } from '@inertiajs/react';
import { Button } from "@/Components/ui/button";

export default function Login() {
    return (
        <div className="min-h-screen w-full flex">
            <Head title="Iniciar Sesión" />

            {/* Left side - Login form */}
            <div className="flex-1 flex items-center justify-center p-8">
                <div className="w-full max-w-[400px] space-y-8">
                    {/* Logo and title */}
                    <div className="text-center space-y-2">
                        <h1 className="text-4xl font-bold flex items-center justify-center gap-1">
                            <span className="text-pink-600">L</span>
                            <span>egatus</span>
                        </h1>
                        <p className="text-gray-500 text-lg">
                            Sistema de Gestión Legal
                        </p>
                    </div>

                    {/* Welcome text */}
                    <div className="text-center space-y-1">
                        <h2 className="text-2xl font-semibold">Bienvenido</h2>
                        <p className="text-gray-500">
                            Inicia sesión para acceder a tu cuenta
                        </p>
                    </div>

                    {/* Google Sign In Button */}
                    <div className="space-y-4">
                        <a href="/auth/google" className="block">
                            <Button
                                type="button"
                                variant="outline"
                                className="w-full h-12 text-base gap-3 border-gray-300 hover:bg-gray-50 hover:border-gray-400 transition-all"
                            >
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="w-5 h-5">
                                    <path
                                        fill="#4285F4"
                                        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                                    />
                                    <path
                                        fill="#34A853"
                                        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                                    />
                                    <path
                                        fill="#FBBC05"
                                        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                                    />
                                    <path
                                        fill="#EA4335"
                                        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                                    />
                                </svg>
                                Continuar con Google
                            </Button>
                        </a>
                    </div>

                    {/* Terms */}
                    <p className="text-center text-xs text-gray-400 pt-4">
                        Al continuar, aceptas nuestros{' '}
                        <a href="#" className="underline hover:text-gray-600">Términos del servicio</a>
                        {' '}y la{' '}
                        <a href="#" className="underline hover:text-gray-600">Política de privacidad</a>.
                    </p>
                </div>
            </div>

            {/* Right side - Gradient background */}
            <div className="hidden lg:block lg:flex-1 relative">
                <div className="absolute inset-0 bg-gradient-to-br from-teal-600 to-lime-400" />
                <div className="absolute inset-0 flex items-center justify-center">
                    <div className="text-white text-center space-y-4 p-8">
                        <h2 className="text-3xl font-bold">Gestiona tus casos legales</h2>
                        <p className="text-lg opacity-90 max-w-md">
                            Organiza clientes, juicios y documentos en un solo lugar
                        </p>
                    </div>
                </div>
            </div>
        </div>
    );
}
